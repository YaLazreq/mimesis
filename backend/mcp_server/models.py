...
class BrandIdentityInfo(BaseModel):
    ...
    primary_color: list[str] = Field(description="Array of primary hex colors. MUST BE PURE HEX STRINGS ONLY (e.g. '#FF0000'). No text.")
    secondary_color: list[str] = Field(description="Array of secondary hex colors. MUST BE PURE HEX STRINGS ONLY. No text.")
...
class BrandPhilosophyInfo(BaseModel):
    ...
    brand_mission: str = Field(description="1 sentence. The core reason the brand exists beyond profit")
    brand_common_enemy: list[str] = Field(description="Array of strings. What does this brand fight against?")
...
class BrandNewsInfo(BaseModel):
    ...
    brand_viral_campaign: list[str] = Field(description="Array of strings with most iconic or recent viral ads")
...
class BrandCultureInfo(BaseModel):
    ...
    brand_symbols: list[str] = Field(description="Array of strings. The brand's main visual symbols, icons, or representations (e.g. Crown for Rolex, fruits for Oasis, running for Nike)")
